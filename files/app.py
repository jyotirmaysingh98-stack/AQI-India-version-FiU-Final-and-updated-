...
Deploy:        see README_DEPLOYMENT.md (Streamlit Community Cloud / HF Spaces)

FOLDER LAYOUT EXPECTED (copy from train_and_export.py's export/ output):
...
from models import (quick_forecast, compute_state_features, cluster_states,
                     manual_augment, realistic_random_augment, scenario_forecast_delta)

st.set_page_config(page_title="AQI India Statewise Forecasting & Learning Platform", layout="wide")
...
def show_img_or_note(path, note):
    if os.path.exists(path):
...
dataset_key = st.sidebar.selectbox(
    "Dataset", list(DATASET_INFO.keys()), format_func=lambda k: DATASET_INFO[k]["label"])
...
state = st.sidebar.selectbox("State / UT", available_states,
...
st.sidebar.markdown("---")
st.sidebar.caption("Statewise SARIMA+LSTM+RF hybrid forecasting · realistic augmentation · "
...
series = city_df.set_index("date")["AQI"].asfreq(
    "D" if info["granularity"].startswith("Daily") else "h").interpolate()

bundle = get_artifact_bundle()
art = bundle.get((dataset_key, state))

st.title("AQI India Statewise Forecasting & Learning Platform")
st.caption(f"Decision-support & educational tool — currently viewing **{state}** on **{info['label']}**")

tab_overview, tab_hist, tab_forecast, tab_aug, tab_live, tab_features = st.tabs(
    ["Overview", "Historical Analysis", "Forecast & Accuracy",
     "Scenario Simulation & Augmentation", "Live 24-Hour Forecast", "Features & Innovation"])
...
with tab_overview:
    ...
    with col1:
        ...
        st.markdown(f"<span style='background:{color};padding:4px 10px;border-radius:6px'>{cat}</span>",
                    unsafe_allow_html=True)
    with col2:
        st.write("**Latest pollutants:**")
        st.dataframe(latest[["PM2.5", "PM10", "NO2", "SO2", "CO", "O3"]].to_frame("value").round(1),
    ...
    overview_rows = [{"Dataset": v["label"], "Granularity": v["granularity"], "Range": v["range"],
                       "Description": v["description"]} for v in DATASET_INFO.values()]
...
with tab_hist:
    ...
    show_img_or_note(f"{EXPORT_OUTPUTS}/seasonal_{dataset_key}_{state}.png",
    ...
    if os.path.exists(path):
        ...
        fig2 = px.bar(sub.sort_values("risk_score", ascending=False), x="state", y="risk_score",
                      color="risk_category", title=f"Statewise Risk Score — {info['label']}")
        st.plotly_chart(fig2, use_container_width=True)
    else:
        feats = compute_state_features(df)
        clustered = cluster_states(feats)
        fig2 = px.bar(clustered.sort_values("risk_score", ascending=False), x="state", y="risk_score",
                      color="risk_category", title="Statewise Risk Score (computed live)")
        st.plotly_chart(fig2, use_container_width=True)
...
with tab_forecast:
    ...
    if os.path.exists(metrics_path):
        ...
        if not sub.empty:
            st.dataframe(sub[["model", "RMSE", "MAE", "MAPE", "R2"]].round(3),
                         use_container_width=True, hide_index=True)
            best = sub.loc[sub.RMSE.idxmin(), "model"]
            st.success(f"**{best}** has the lowest RMSE for {state} on {info['label']}.")
            st.subheader("Cross-Dataset Comparison (same state, all datasets trained)")
            cross = m[m.state == state]
            fig3 = px.bar(cross, x="dataset", y="RMSE", color="model", barmode="group",
                          title=f"RMSE by dataset & model — {state}")
            st.plotly_chart(fig3, use_container_width=True)
        else:
            st.info(f"No pre-trained metrics for {state}/{dataset_key} yet — "
                    f"add it to REP_STATES in train_and_export.py and re-run.")
    else:
        st.info("Run train_and_export.py to populate the full accuracy dashboard. "
                "Showing a quick on-demand SARIMA forecast below in the meantime.")

    st.subheader("Forecast vs Actual (hybrid model)")
    show_img_or_note(f"{EXPORT_OUTPUTS}/forecast_{dataset_key}_{state}.png",
    ...
    try:
        fc = quick_forecast(series, steps=horizon, freq="D" if info["granularity"].startswith("Daily") else "H")
        figf = go.Figure()
        figf.add_trace(go.Scatter(x=series.index[-60:], y=series.values[-60:], name="Recent actual"))
        figf.add_trace(go.Scatter(x=fc["timestamp"], y=fc["forecast"], name="Forecast", line=dict(dash="dash")))
        figf.add_trace(go.Scatter(x=fc["timestamp"], y=fc["upper_80"], line=dict(width=0), showlegend=False))
        figf.add_trace(go.Scatter(x=fc["timestamp"], y=fc["lower_80"], line=dict(width=0), fill="tonexty",
                                   fillcolor="rgba(255,127,14,0.2)", name="80% band"))
...
with tab_aug:
    ...
    fig4 = go.Figure()
    fig4.add_trace(go.Scatter(x=series.index[-120:], y=series.values[-120:], name="Original"))
    fig4.add_trace(go.Scatter(x=augmented.index[-120:], y=augmented.values[-120:], name="Augmented"))
    fig4.update_layout(title="Original vs Augmented AQI Series (last 120 points)")
    ...
    try:
        ...
        cmp_df = pd.DataFrame({"day": range(1, 8), "baseline_forecast": baseline_fc["forecast"].values,
                                "scenario_forecast": scenario_fc["forecast"].values})
        cmp_df["delta"] = cmp_df["scenario_forecast"] - cmp_df["baseline_forecast"]
        st.dataframe(cmp_df.round(1), use_container_width=True, hide_index=True)
        fig5 = go.Figure()
        fig5.add_trace(go.Scatter(x=cmp_df["day"], y=cmp_df["baseline_forecast"], name="Baseline"))
        fig5.add_trace(go.Scatter(x=cmp_df["day"], y=cmp_df["scenario_forecast"], name="Scenario"))
        st.plotly_chart(fig5, use_container_width=True)
        if "saved_scenarios" not in st.session_state:
            st.session_state["saved_scenarios"] = []
        if st.button("💾 Save this scenario for comparison"):
            st.session_state["saved_scenarios"].append(
                {"state": state, "dataset": dataset_key, **pct_changes, "avg_delta": cmp_df["delta"].mean()})
...
with tab_live:
    ...
    st.caption("Pulls the latest real readings (live API if configured, else a fresh synthetic "
    ...
    st.write(f"**Last updated:** {pd.Timestamp.now():%Y-%m-%d %H:%M} "
             f"(source: `{live_df['source'].iloc[0]}`)")
    latest_live = live_df.sort_values("date").iloc[-1]
    lc1, lc2 = st.columns(2)
    with lc1:
        st.metric("Latest observed AQI (live)", f"{latest_live['AQI']:.0f}")
    with lc2:
        st.dataframe(latest_live[["PM2.5", "PM10", "NO2", "SO2", "CO", "O3"]].to_frame("value").round(1),
                     use_container_width=True)
    ...
    try:
        ...
        figl = go.Figure()
        figl.add_trace(go.Scatter(x=live_series.index, y=live_series.values, name="Last 24h observed"))
        figl.add_trace(go.Scatter(x=live_fc["timestamp"], y=live_fc["forecast"], name="Next 24h forecast",
                                   line=dict(dash="dash")))
        figl.add_trace(go.Scatter(x=live_fc["timestamp"], y=live_fc["upper_80"], line=dict(width=0), showlegend=False))
        figl.add_trace(go.Scatter(x=live_fc["timestamp"], y=live_fc["lower_80"], line=dict(width=0),
                                   fill="tonexty", fillcolor="rgba(0,150,255,0.2)", name="80% band"))
        figl.update_layout(title="Next 24-Hour AQI Forecast (hourly)")
        st.plotly_chart(figl, use_container_width=True)
    except Exception as e:
        st.warning(f"Live forecast unavailable: {e}")

    st.caption("To use a real live feed: set `DATA_GOV_API_KEY` as a Streamlit secret and update "
               "`REALTIME_API_URL` in data_loader.py to your data.gov.in/CPCB/AIKosh resource.")
...
8. **Educational mode** (this tab + Overview) — explains dataset differences and model mechanics so the
...
st.markdown("---")
st.caption("SARIMA (statsmodels) + LSTM/BiLSTM (TensorFlow) + RF/XGBoost correction · KMeans risk clustering · "
           "SHAP explainability · live data integration. Train with train_and_export.py in Colab.")
